...
pipeline, verification framework, and quantum game engine. Every module targets
**≤ 400 LOC** (excluding tests). Each step lists the files it creates/modifies,
what it tests, and its dependencies.
...
## Phase 0 — Call Graph Redesign & Gate Count Storage (PRD Milestone 0)
...
```
Phase 0 (Call Graph Redesign)  ← current milestone, no downstream blockers
  ├── 0.1–0.4 (Python: edges, flat DAG, freeze)  ─┐
  └── 0.5–0.6 (C: sequence gate counts)           ─┤── 0.7 (wire counts)
                                                    └── Integration test

Phase 1 (Classical Verification)  [independent of Phase 0]
