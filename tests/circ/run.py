...
for s in sizes:
    print(s)
    res = subprocess.run(["/usr/bin/time", "-l", "/Users/sorenwilkening/.pyenv/versions/3.11.9/envs/qiskit/bin/python3", "cirq_bashing.py", f"{s}"], capture_output=True, text = True)

    t1 = float(res.stdout)
    mem = int(res.stderr.split("maximum resident set size")[0].split("\n")[-1])

    with open("circ.csv", "a") as f:
        f.write(f"{s},{t1},{mem}\n")
        f.close()