...
hypothesis: CONFIRMED -- gates_are_inverse() in optimizer.c incorrectly treats two identical Ry(pi/2) gates as inverses, causing cancellation
test: Static analysis of gates_are_inverse() logic for Ry gates with same angle
expecting: Function returns true for two Ry(pi/2) gates, causing add_gate() to cancel both
...
root_cause: gates_are_inverse() in c_backend/src/gate.c (line 548) incorrectly treats rotation gates (Ry, Rx, Rz) with identical angles as inverses. The check `G1->GateValue != G2->GateValue` returns false when both angles are equal (e.g., pi/2), causing the function to return true. The optimizer's add_gate() then cancels both gates via merge_gates(), leaving zero gates in the circuit. The correct inverse check for rotation gates should verify NEGATED angles: Ry(theta)^{-1} = Ry(-theta), NOT Ry(theta). The P gate already has the correct logic (`G1->GateValue != -G2->GateValue`), but Ry/Rx/Rz do not.
fix:
verification:
files_changed: []
